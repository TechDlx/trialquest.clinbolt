...
function QuestScene() {
  return (
    <div className="relative mx-auto w-full max-w-[350px]">
      <svg
        viewBox="0 0 350 240"
        className="block h-auto w-full text-brand-700 dark:text-brand-300"
        aria-hidden="true"
      >
      ...
      <span
        className="absolute aspect-square rounded-full ring-4 ring-surface"
...
function Tile({
  ...
  return (
    <li className={`flex items-center gap-2 rounded-2xl p-2.5 ${skin}`}>
      ...
      <span className="flex min-w-0 flex-col gap-0.5">
        ...
        <span className={`text-xs leading-[1.4] text-slate-700 ${body}`}>{children}</span>
      </span>
    </li>
  );
}
...
export function TitleScreen() {
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-3.5 px-4 py-5 safe-top safe-bottom">
      <header className="flex items-center justify-between gap-3">
        <span className="inline-flex rounded-xl dark:bg-white dark:px-2 dark:py-1">
          <img src={logoUrl} alt="ClinBolt" width="122" height="30" className="block h-[30px] w-auto" />
        </span>
        <span className="rounded-full border-2 border-brand-600 px-3.5 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.14em] text-brand-700 dark:border-brand-300 dark:text-brand-300">
          Learn by playing
        </span>
      </header>

      <div className="flex flex-col gap-2">
        <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-brand-700 dark:text-brand-300">
          A clinical trial adventure
        </p>
        <h1 className="text-[clamp(2.75rem,13vw,3.625rem)] font-black leading-none tracking-tight text-brand-800 dark:text-fg">
          Trial Quest
        </h1>
        <p className="text-[clamp(1.5rem,7vw,1.75rem)] font-extrabold leading-tight text-brand-600 dark:text-brand-300">
          From molecule
          <br />
          to medicine.
        </p>
        <p className="max-w-[20rem] text-[17px] leading-relaxed">
      ...
      {hasProgress ? (
        <>
          ...
          <Button variant="ghost" onClick={() => navigate({ name: 'intro' })} className="self-center">
            Replay the intro
          </Button>
        </>
      ) : (
